...
class PantryPilotBootstrap:
    ...
    def build_state(self) -> dict[str, Any]:
        ...
        trip = self._build_trip(pantry_items, restock_plan, offer_items, aisle_map)
        ...
        report = self._build_report(report_context)
    ...
    def _build_inventory_snapshot(self, raw_items: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            return serializers.normalize_inventory_snapshot(build_method(raw_items), raw_items)
        except Exception:
    ...
    def _build_restock_plan(
        ...
        try:
            return serializers.normalize_restock_plan(build_method(raw_items, catalog, preferences))
        except Exception:
            self.logger.exception("RestockAdvisor.build_plan failed. Using fallback restock suggestions.")
            return fallback_value

    def _build_pantry_items(self, raw_items: list[dict[str, Any]]) -> list[Any]:
        ...
        return converted_items or fallback_items

    def _build_offers(self, pantry_items: list[Any]) -> list[dict[str, Any]]:
        ...
        try:
            return serializers.normalize_offers(build_method(pantry_items))
        except Exception:
    ...
    def _build_trip(
        ...
        try:
            return serializers.normalize_trip(build_method(pantry_items, restock_plan, offers, aisle_map))
        except Exception:
            self.logger.exception("ShoppingPlanner.build_trip failed. Using fallback shopping trip.")
            return fallback_value

    def _build_report(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return serializers.normalize_report(build_method(context))
        except Exception:
